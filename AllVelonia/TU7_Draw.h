namespace TU7
{
	...
	void drawNewsBar() {
		...
		drawText(va("%s", _newsbarTxt), _newsbarxPos, 714, smallFont, 0.8, white);
	...
	void drawLine(float X1, float Y1, float X2, float Y2, float Width, float* Colors) {
		...
		CG_DrawRotatedPicPhysical(0x8274F580, X, Y, L2, Width, Angle, Colors, Material_RegisterHandle("white", 0));
	}
	...
	inline void drawInfo() {
		...
		GameInfo(va("Connected Clients: %d", _level_locals->numConnectedClients), 5, pos + (27 * 7), "fonts/bigFont", .3f, white);
		GameInfo(va("Entity Count: %d", _level_locals->numEntities), 5, pos + (27 * 8), "fonts/bigFont", .3f, white);
	}
