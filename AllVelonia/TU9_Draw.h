namespace TU9
{
	...
	void drawLine(float X1, float Y1, float X2, float Y2, float Width, float* Colors) {
		...
		CG_DrawRotatedPicPhysical(0x82690F40, X, Y, L2, Width, Angle, Colors, Material_RegisterHandle("white", 0));
	}
	...
	void draw3DBox(int client, float* sColor) {
		Vector3 Center = Centity_s[client].mPos;
	...
	void drawTriBox(int client, float* sColor) {
		Vector3 Center = Entities[client]->mPos;
		float W = GetBoxWidth(&Centity_s[client]), H = GetBoxHeight(&Centity_s[client]);
		W *= 1.5; H *= 1.25;
	...
	char* hasClanTag(int id) {
		auto finalName = cg->ClientInfo[id].mName;
		if (*cg->ClientInfo[id].clanTag != 0) finalName = va("[%s]%s", cg->ClientInfo[id].clanTag, finalName);
		return finalName;
	}

	char* calculateServerInfo() {
