namespace TU17
{
	...
	void drawShader(float x, float y, float w, float h, float* Color, char* material) {
		R_AddCmdDrawStretchPic(x, y, w, h, 0, 0, 1, 1, Color, reinterpret_cast<void*>(Material_RegisterHandle(material, 0)));
	}
	...
	void drawText(const char* text, float x, float y, char* font, float fontSize, float* Color) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), x, y, fontSize, fontSize, 0, Color, 0);
	}
	...
	void drawRightText(const char* text, float x, float y, char* font, float fontSize, float* Color) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 1) * fontSize), y, fontSize, fontSize, 0, Color, 0);
	}

	void drawCenterText(const char* text, float x, float y, char* font, float fontSize, float* Color) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 2) * fontSize), y, fontSize, fontSize, 0, Color, 0);
	}

	void drawShadowText(const char* text, float x, float y, char* font, float fontSize, float* Color, float* sColor) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), x + 1, y + 1, fontSize, fontSize, 0, sColor, 0);
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), x, y, fontSize, fontSize, 0, Color, 0);
	}

	void drawRightShadowText(const char* text, float x, float y, char* font, float fontSize, float* Color, float* sColor) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 1) * fontSize) + 1, y + 1, fontSize, fontSize, 0, sColor, 0);
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 1) * fontSize), y, fontSize, fontSize, 0, Color, 0);
	}

	void drawCenterShadowText(const char* text, float x, float y, char* font, float fontSize, float* Color, float* sColor) {
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 2) * fontSize) + 1, y + 1, fontSize, fontSize, 0, sColor, 0);
		R_AddCmdDrawText(text, 0x7FFFFFFF, (void*)R_RegisterFont(font, 0), (x - (TextWidth(text, font) / 2) * fontSize), y, fontSize, fontSize, 0, Color, 0);
	}
	...
	void drawNewsBar() {
		...
		drawText(va("%s", _newsbarTxt), _newsbarxPos, 714, smallFont, .75, white);
	...
	void drawLine(float X1, float Y1, float X2, float Y2, float Width, float* Colors) {
		...
		CG_DrawRotatedPicPhysical(0x82BCDD10, X, Y, L2, Width, Angle, Colors, Material_RegisterHandle("white", 0));
	}
	...
	{
		const auto textWid = R_TextWidth(text, 0x7FFFFFFF, R_RegisterFont(bigFont, 0)) * fontSize;
	...
	void drawInfo() {
		getStructs();
		const auto pos = 210;
		GameInfo(va("Map: %s", UI_GetMapDisplayName(Dvar_GetString("ui_mapname"))), 5, pos + (27 * 0), bigFont, .2f, white);
		GameInfo(va("Game: %s", UI_GetGameTypeDisplayName(cgs->GameType)), 5, pos + (27 * 1), bigFont, .2f, white);
		GameInfo(va("FPS: %.0f", (UI.FPS / 2)), 5, pos + (27 * 2), bigFont, .2f, white);
		GameInfo(va("Host: %s", cgs->Host), 5, pos + (27 * 3), bigFont, .2f, white);
		GameInfo(va("Ping: %d", snapshot.ping), 5, pos + (27 * 4), bigFont, .2f, white);
		GameInfo(va("Health: %d", cg->Health), 5, pos + (27 * 5), bigFont, .2f, white);
		GameInfo(va("Connected Clients: %d", snapshot.connectedClients), 5, pos + (27 * 6), bigFont, .2f, white);
		GameInfo(va("Entity Count: %d", snapshot.entityCount), 5, pos + (27 * 7), bigFont, .2f, white);
	}

	void drawConsoleInfo() {
		GameInfo(va("Input: %s", Console.GetAVPackName()), 99, 13 + (27 * 0), smallFont, .3f, white);
		GameInfo(va("CPU: %.2f", Console.GetTemperature(CPU, true)), 172, 13, smallFont, .3f, white);
		GameInfo(va("GPU: %.2f", Console.GetTemperature(GPU, true)), 233, 13, smallFont, .3f, white);
		GameInfo(va("Mem: %.2f", Console.GetTemperature(MEM, true)), 295, 13, smallFont, .3f, white);
		GameInfo(va("Case: %.2f", Console.GetTemperature(BRD, true)), 359, 13, smallFont, .3f, white);
	}
}