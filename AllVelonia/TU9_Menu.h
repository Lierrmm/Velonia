namespace TU9
{
	...
	void addiSlider(int i, const char* text, int var, int min, int max) {
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(0x82690F40, 1114 + 1, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandle("white", 0));
	...
	void addiSliderOption(int i, const char* text, int var, int min, int max, bool selected)
	{
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(0x82690F40, 1115, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandle("white", 0));
	...
	void MenuStructure() {
		...
		drawRightText("TU9", 1235, 48, normalFont, .5, white);
		...
		case PLAYERS:
			auto p = 0;
			for (auto i = 0; i < 18; i++) {
				if (cg->ClientInfo[i].Valid & 1) {
					addOption(p, va("[%s] %s - %s", checkLocalClient(i) ? "^3YOU^7" : checkFFAMode() ? "^1E^7" : notTeam(i) ? "^1E^7" : "^2F^7", cg->ClientInfo[i].mName, getIpAsString(i)), .46);
					p++;
				}
			}
			break;
