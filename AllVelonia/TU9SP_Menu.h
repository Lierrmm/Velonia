...
namespace TU9SP
{
	...
	void addiSlider(int i, const char* text, int var, int min, int max) {
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(srcPlacement, 1114 + 1, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandle("white", 0));
	...
	void addiSliderOption(int i, const char* text, int var, int min, int max, bool selected)
	{
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(srcPlacement, 1115, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandle("white", 0));
		...
		case PLAYERS:
			...
			for (auto i = 0; i < 2; i++) {
				if (level.clients[i].pers.connected == CON_CONNECTED) {
					addOption(p, va("[%s] %s - %s", checkLocalClient(i) ? "^3YOU^7" :"^2F^7", level.clients[i].pers.playerName, "0.0.0.0"), .46);
					p++;
				}
			}
			break;
