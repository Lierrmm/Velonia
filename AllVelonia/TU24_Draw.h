...
namespace TU24
{
	...
	void drawLine(float X1, float Y1, float X2, float Y2, float Width, float* Colors) {
		...
		CG_DrawRotatedPicPhysical(0x827322C0, X, Y, L2, Width, Angle, Colors, Material_RegisterHandleInt("white", 0));
	}
	...
	inline void draw3DBounded(Vector3 top, Vector3 bot, float a, int width, int thickness, float* color)
	{
	...
	inline void Draw2DBox(Vector2 top, Vector2 bot, int thickness, float* color)
	{
	...
	inline void drawPyramid(Vector3 center, float w, float h, float* color)
	{
		MakeESPLine(center, -w, -w, 0, w, -w, 0, color, 1);
		MakeESPLine(center, -w, -w, 0, -w, w, 0, color, 1);
		MakeESPLine(center, w, w, 0, w, -w, 0, color, 1);
	...
	inline void drawLines(Vector3 Origin, float X1, float Y1, float Z1, float X2, float Y2, float Z2, float* Color, int Size) {
	...
	inline void drawBoneLine(pCentity_t entity, int ent, char* tagname1, char* tagname2, float* color) {
		auto Origin1 = GetTagPos(ent, tagname1), Origin2 = GetTagPos(ent, tagname2);
		Vector2 Loc1, Loc2;
	...
	inline void doAllBones(int Client, float* boneESPCol) {
		auto ent = &Centity_s[Client];
		auto entNum = Client;
	...
	inline void draw3DBox(int client, float* sColor) {
		auto Center = Centity_s[client].origin;
		auto W = GetBoxWidth(&Centity_s[client]), H = GetBoxHeight(&Centity_s[client]);
	...
	inline void drawTriBox(int client, float* sColor) {
		auto Center = Centity_s[client].origin;
		auto W = GetBoxWidth(&Centity_s[client]), H = GetBoxHeight(&Centity_s[client]);
		W *= 1.5; H *= 1.25;
	...
	inline void drawBoundingBox(Vector3 HeadOrigin, float W, float H, float* sColor) {
	...
	inline void GameInfo(char* text, float x, float y, char* font, float fontSize, float* color)
	{
	...
	inline void GameInfoNoPadding(char* text, float x, float y, char* font, float fontSize, float* color) {
		const auto textWid = R_TextWidth(text, 0x7FFFFFFF, R_RegisterFont("fonts/bigFont", 0)) * fontSize;
	...
	inline char* hasClanTag(int id) {
		auto* finalName = cg->ClientInfo[id].szPlayerName;
		if (*cg->ClientInfo[id].clanTag != 0) finalName = va("[%s]%s", cg->ClientInfo[id].clanTag, finalName);
		return finalName;
	}

	inline char* calculateServerInfo() {
		auto* host = clientStatic.servername;
		if (strcmp(clientStatic.servername, "localhost")) {
			for (auto i = 0; i < snapshot.connectedClients; i++) {
				if (!strcmp(cgs->Host, hasClanTag(i))) {
					host = getIpAsString(i);
					break;
				}
			}
		}
	...
	inline void drawInfo() {
		...
		GameInfo(va("Server: %s", calculateServerInfo()), 5, pos + (27 * 4), "fonts/bigFont", .3f, white);
		GameInfo(va("Ping: %d", snapshot.ping), 5, pos + (27 * 5), "fonts/bigFont", .3f, white);
		GameInfo(va("Health: %d", cg->Health), 5, pos + (27 * 6), "fonts/bigFont", .3f, white);
		GameInfo(va("Connected Clients: %d", snapshot.connectedClients), 5, pos + (27 * 7), "fonts/bigFont", .3f, white);
		GameInfo(va("Entity Count: %d", snapshot.entityCount), 5, pos + (27 * 8), "fonts/bigFont", .3f, white);
	}

	inline void drawConsoleInfo() {
