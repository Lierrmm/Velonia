#ifndef TU24SP_MENU_H
#define TU24SP_MENU_H
...
namespace TU24SP
{
	...
	void addiSlider(int i, const char* text, int var, int min, int max) {
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(0x8278BE88, 1114 + 1, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandleInt("white", 0));
	...
	void addiSliderOption(int i, const char* text, int var, int min, int max, bool selected)
	{
		...
		addOption(i, text);
		CG_DrawRotatedPicPhysical(0x8278BE88, 1115, 29 + ((i + 1) * 28) + 2, X, 8, 0, Smoke, Material_RegisterHandleInt("white", 0));
	...
	void MenuStructure() {
...
#if defined(_debug)
			drawMenuOption(8, "^:DEBUG");
#endif
			break;
		case SELF:
			addiSlider(0, "Change Team", customTeam, 1, 3);
			addiSlider(1, "Change Class", customClass, 1, 10);
			addOption(2, "Give Juggernaut");
			addBool(3, "Unlimited Radar", radar);
			addBool(4, "No Recoil", recoil);
			addBool(5, "No Sway", sway);
			addBool(6, "No Flinch", noflinch);
			addBool(7, "No Flash", flashing);
			addBool(8, "No Infect", _noinfect);
		...
		case PLAYERS:
			auto p = 0;
			for (auto i = 0; i < 2; i++) {
				if (bgs.clientinfo[cg.clientNum].valid & 1) {
					addOption(p, va("[%s] %s - %s", checkLocalClient(i) ? "^3YOU^7" : "^2F^7", Lobby_GetName(i), getIpAsString(i)), .46);
					p++;
				}
			}
			break;
	...
	void SelectMonitor() {
		...
		case MAIN:
			...
			case 8: CE.curMenu = DEBUG; CE.curScroll = 0; break;
			}
			break;
		case SELF:
			switch (CE.curScroll) {
			case 0:
				*reinterpret_cast<byte*>(0xC328AE6A) = 0xFF; //GODMODE Temp
				*reinterpret_cast<byte*>(0xC328AE6B) = 0xFF;
				break;
			/*case 0: togteam(); break;
			case 1: changeClass(); break;
			case 2: GiveJug(); break;
			case 3: giveRadar(); break;
			case 4: Recoil(); break;
			case 5: weaponSway(); break;
			case 6: noFlinch(); break;
			case 7: stopFlash(); break;
			case 8: _noinfect = !_noinfect; break;
