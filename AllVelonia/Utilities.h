#pragma once
#include "stdafx.h"

#define FLAG_DEAD 0x02
#define FLAG_CROUCHED 0x04
#define FLAG_PRONE 0x08
#define AngleToShort(X) ((DWORD)((X) * 65536 / 360) & 65535)
#define ANGLE2SHORT(x) ((int)((x) * 0x10000 / 360) & 0xFFFF)
#define ANGLE2SHORTFLOAT(x) ((short)((x)*65536/360) & 65535)
#define	SHORT2ANGLE(x)	((x) * (360.0/65536.0f))

#define GetPtr(X) *(PDWORD)(X)
#define M_PI 3.14159265358979323846f
#define XINPUT_GAMEPAD_LEFT_TRIGGER		Input.Gamepad.bLeftTrigger
#define XINPUT_GAMEPAD_RIGHT_TRIGGER	Input.Gamepad.bRightTrigger
#define VectorSubtract(a,b,c)	((c)[0]=(a)[0]-(b)[0],(c)[1]=(a)[1]-(b)[1],(c)[2]=(a)[2]-(b)[2])
#define TORAD(x)  ((x) * 0.01745329252)

int XNADDRA(int client) {
	int num = XamGetCurrentTitleId();
	int currentTitle = num;
	int num2 = num;
	if (num2 <= 1096157379u) {
		if (num2 <= 1096157207u) {
			if (num2 == 1096157158u) return 2185484032u + client * 184u;
			if (num2 == 1096157207u) return 2187914946u + client * 192u;
		}
		else {
			if (num2 == 1096157269u) return 2191492176u + client * 512u;
			if (num2 == 1096157379u) return 2194326432u + client * 328u;
		}
	}
	else if (num2 <= 1096157436u) {
		if (num2 == 1096157387u) return 2188513004u + client * 344u;
		if (num2 == 1096157436u) return 2193348148u + client * 448u;
	}
	else {
		if (num2 == 1096157460u) return 2193090112u + client * 848u;
		if (num2 == 1096157469u) return 3298693576u + client * 264u;
	}
}

int GetIP(int address) {
	int num = XamGetCurrentTitleId();
	int currentTitle = num;
	int num2 = num;
	if (num2 <= 1096157379u) {
		if (num2 <= 1096157207u) {
			if (num2 == 1096157158u) return address + 88u;
			if (num2 == 1096157207u) return address + 66u;
		}
		else {
			if (num2 == 1096157269u) return address + 96u;
			if (num2 == 1096157379u) return address + 124u;
		}
	}
	else if (num2 <= 1096157436u) {
		if (num2 == 1096157387u) return address + 184u;
		if (num2 == 1096157436u) return address + 232u;
	}
	else {
		if (num2 == 1096157460u) return address + 668u;
	}
}

class Utils {
public:
	static PWCHAR charToWChar(__in LPCSTR c_buffer);
	static VOID XNotifyUI(PWCHAR pwszStringParam);
	static VOID XNotifyDoQueueUI(PWCHAR pwszStringParam);
	static HRESULT SetMemory(VOID* Destination, VOID* Source, DWORD Length);
	static HRESULT GetMemory(VOID* Destination, VOID* Source, DWORD Length);
	static VOID HookFunctionStart(PDWORD Address, PDWORD SaveStub, DWORD Destination);
	static VOID PatchInJump(DWORD* Address, DWORD Destination, BOOL Linked = false);
	static VOID PatchInBranch(DWORD* Address, DWORD Destination, BOOL Linked);
	static FARPROC ResolveFunction(CHAR* ModuleName, DWORD Ordinal);
	static DWORD PatchModuleImport(CHAR* Module, CHAR* ImportedModuleName, DWORD Ordinal, DWORD PatchAddress);
	static DWORD PatchModuleImport(PLDR_DATA_TABLE_ENTRY Module, CHAR* ImportedModuleName, DWORD Ordinal, DWORD PatchAddress);
	static INT toWCHAR(char* input, WCHAR* output);
	static BOOL IsTrayOpen();
	static VOID XexThread(LPTHREAD_START_ROUTINE Thread);
	static INT snprintf(char* buffer, size_t len, const char* fmt, ...);
};

typedef HRESULT(*pDmSetMemory)(LPVOID lpbAddr, DWORD cb, LPCVOID lpbBuf, LPDWORD pcbRet);

#define HvxPeekPokeExpID 0x48565050

#define HvxProcedureExpID 0x48565051

#define HvxCall QWORD __declspec(naked)

enum PEEK_POKE_TYPE : DWORD {
	PEEK_BYTE = 0,
	PEEK_WORD = 1,
	PEEK_DWORD = 2,
	PEEK_QWORD = 3,
	PEEK_BYTES = 4,
	POKE_BYTE = 5,
	POKE_WORD = 6,
	POKE_DWORD = 7,
	POKE_QWORD = 8,
	POKE_BYTES = 9,
	PEEK_SPR = 0xA
};

CONST BYTE HvxPeekPokeExp[0x2F0] = {
	
};

static HvxCall HvxExpansionInstall(DWORD PhysicalAddress, DWORD CodeSize)
{
	__asm
	{
		li r0, 0x72
		sc
		blr
	}
}

static HvxCall HvxExpansionCall(DWORD ExpansionID, QWORD r4 = 0, QWORD r5 = 0, QWORD r6 = 0, QWORD r7 = 0)
{
	__asm
	{
		li r0, 0x73
		sc
		blr
	}
}

extern BYTE HvxPeekBYTE(QWORD Address);
extern WORD HvxPeekWORD(QWORD Address);
extern DWORD HvxPeekDWORD(QWORD Address);
extern QWORD HvxPeekQWORD(QWORD Address);

class MemoryBuffer {
public:
	MemoryBuffer(DWORD dwSize = 512) {
		m_pBuffer = NULL;
		m_dwDataLength = 0;
		m_dwBufferSize = 0;
		if ((dwSize < UINT_MAX) && (dwSize != 0)) {
			m_pBuffer = (BYTE*)malloc(dwSize + 1);
			if (m_pBuffer) {
				m_dwBufferSize = dwSize;
				m_pBuffer[0] = 0;
			}
		}
	};
	~MemoryBuffer() {
		if (m_pBuffer)
			free(m_pBuffer);
		m_pBuffer = NULL;
		m_dwDataLength = 0;
		m_dwBufferSize = 0;
	};
	BOOL Add(const void* p, DWORD dwSize) {
		if (CheckSize(dwSize)) {
			memcpy(m_pBuffer + m_dwDataLength, p, dwSize);
			m_dwDataLength += dwSize;
			*(m_pBuffer + m_dwDataLength) = 0;
			return TRUE;
		}
		else
			return FALSE;
	};
	BYTE* GetData() const {
		return m_pBuffer;
	};
	DWORD GetDataLength() const {
		return m_dwDataLength;
	};
	VOID Rewind() {
		m_dwDataLength = 0; m_pBuffer[0] = 0;
	};
	BOOL CheckSize(DWORD dwSize) {
		if (m_dwBufferSize >= (m_dwDataLength + dwSize))
			return TRUE;
		else {
			DWORD dwNewSize = max(m_dwDataLength + dwSize, m_dwBufferSize * 2);
			BYTE* pNewBuffer = (UCHAR*)realloc(m_pBuffer, dwNewSize + 1);
			if (pNewBuffer) {
				m_pBuffer = pNewBuffer;
				m_dwBufferSize = dwNewSize;
				return TRUE;
			}
			else
				return FALSE;
		}
	}
private:
	BYTE* m_pBuffer;
	DWORD m_dwDataLength;
	DWORD m_dwBufferSize;
};

BOOL FileExists(PCHAR Path);
BOOL CReadFile(CONST PCHAR FilePath, MemoryBuffer& Buffer);
BOOL CWriteFile(CONST PCHAR FilePath, CONST PVOID Buffer, DWORD Size);
BOOL ReadFileAlt(PCHAR Path, LPVOID Buffer, DWORD dwBytesToRead);
BOOL WriteFileAlt(PCHAR Path, LPCVOID Buffer, DWORD dwBytesToWrite);
BOOL CWriteFile(const CHAR* FilePath, const VOID* Data, DWORD Size);

extern DWORD MountPath(PCHAR Drive, PCHAR Device);
extern BOOL InitializeHvxPeekPoke();